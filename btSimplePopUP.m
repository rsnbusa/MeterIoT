...
//#define SCREEN_SIZE chartView.bounds

//#define POPUP_WIDTH 500.0
//#define POP_HEIGHT 500.0
...
-(void)commonInitWithImage:(UIImage *)image andTitle:(NSString *)aTitle andFrame:(CGRect) frame{
    ...
    title.font = [UIFont fontWithName:@"Avenir Next" size:([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad)?20:10];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        POPUP_WIDTH= ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad)?410:250;
        POP_HEIGHT= ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad)?410:250;
...
-(instancetype)initWithItemImage:(NSArray *)items andTitles:(NSArray *)titleArray addToViewController:(UIViewController*)sender {
    ...
    return [self initWithItemImage:items andTitles:titleArray andActionArray:nil addToViewController:sender andSize:[UIScreen mainScreen].bounds.size];
}
...
-(instancetype)initWithItemImage:(NSArray *)items andActionArray:(NSArray *)actionArray addToViewController:(UIViewController *)sender  {
    return [self initWithItemImage:items andTitles:nil andActionArray:actionArray addToViewController:sender andSize:[UIScreen mainScreen].bounds.size];
}

-(instancetype)initWithItemImage:(NSArray *)items andTitles:(NSArray *)titleArray andActionArray:(NSArray *)actionArray addToViewController:(UIViewController*)sender andSize:(CGSize)theSize{
    self = [super init];
    if (self) {
...
-(void)setUpPopItems {
    itemSize=([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad)?CGSizeMake(90.0f, 90.0f):CGSizeMake(30.0f, 30.0f);
    itemTextColor = [UIColor whiteColor];
    itemFont =[UIFont boldSystemFontOfSize: ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad)?44.f:14.0f];
            ...
            [UIView animateWithDuration:0.6
                             animations:^{
                               ...
                               //  contentView.frame = CGRectMake(self.frame.size.width/2-150, SCREEN_SIZE.size.height, POPUP_WIDTH, POP_HEIGHT);
                                 contentView.frame = CGRectMake(self.frame.size.width/2-150, workSize.height, POPUP_WIDTH, POP_HEIGHT);
